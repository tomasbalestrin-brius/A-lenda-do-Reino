...
export function computeStats(char) {
  try {
    if (char.system === 'dnd5e') {
      return computeStatsDND(char);
    }
    return computeStatsT20(char);
  } catch (error) {
...
function computeStatsDND(char) {
  ...
  const raceData = RACES_DND5E[char.raca?.toLowerCase()];
  if (raceData && raceData.atributos) {
  ...
  // 3. Pontos de Vida (PV)
  const classData = CLASSES_DND5E[char.classe?.toLowerCase()];
