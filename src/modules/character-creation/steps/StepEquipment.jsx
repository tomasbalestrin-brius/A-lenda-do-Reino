// Domínio: character-creation | Dono ÚNICO de: StepEquipment.jsx
...
export function StepEquipment() {
  ...
  }, [char.classe, char.levelChoices]);

  const checkProficiency = (item) => {
    if (!item) return true;
    if (item.tipo === 'arma') {
      const cat = item.categoria || 'simples';
      if (cat === 'simples') return true; // Todos são treinados em armas simples (geralmente)
      return proficiencies.has('Armas Marciais');
    }
    if (item.tipo === 'armadura') {
      if (item.categoria === 'pesada') return proficiencies.has('Armaduras Pesadas');
      return true; // Leve é padrão (geralmente)
    }
    if (item.tipo === 'escudo') return proficiencies.has('Escudos');
    return true;
  };
    ...
    <div className="flex flex-col gap-10">
      <div className="flex flex-col md:flex-row items-center justify-between gap-6 bg-amber-950/20 p-6 md:p-10 rounded-[2rem] md:rounded-[3rem] border border-amber-500/10 shadow-2xl relative overflow-hidden backdrop-blur-md">
        ...
        <div className="flex flex-col items-stretch md:items-end gap-3 w-full md:w-auto">
            <div className="px-6 py-4 bg-gray-950 border-2 border-amber-500/20 rounded-[2rem] flex flex-col items-center justify-center min-w-[180px] shadow-2xl relative group overflow-hidden">
                <div className="absolute inset-0 bg-gradient-to-br from-amber-500/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
                <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1 relative z-10">Patrimônio Disponível</span>
                {editingGold ? (
                  <input autoFocus type="number" min="0" defaultValue={char.dinheiro || 0}
            ...
            <div className="flex flex-col gap-1.5 px-2">
              {(() => {
                const currentLoad = (char.equipamento || []).reduce((acc, e) => {
                  ...
                  else if (['consumivel', 'pergaminho', 'alquimico', 'comida'].includes(item.tipo)) weight = 0.5;
                  if (isMitral || isDiscreto) weight = Math.max(0.5, weight - 1);
                  return acc + weight;
                }, 0);
                const pct = Math.min(100, (currentLoad / stats.maxLoad) * 100);
                const colorClass = pct > 90 ? 'bg-rose-500' : pct > 70 ? 'bg-amber-500' : 'bg-emerald-500';
                return (
                  <>
                    <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest text-slate-500">
                      <span>Carga do Herói</span>
                      <span className={pct > 100 ? 'text-rose-500' : 'text-slate-300'}>{currentLoad} / {stats.maxLoad} kg</span>
                    </div>
                    <div className="h-2 bg-gray-900 rounded-full overflow-hidden border border-white/5">
                      <motion.div initial={{ width: 0 }} animate={{ width: `${pct}%` }} className={`h-full ${colorClass} shadow-[0_0_10px_rgba(0,0,0,0.5)] transition-colors duration-500`} />
                    </div>
                  </>
                );
              })()}
          ...
          <div className={`grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 ${category === 'magico' ? 'hidden' : ''}`}>
            <AnimatePresence>
              {filteredItems.map(item => {
                const isOwned = (char.equipamento || []).some(e => (typeof e === 'string' ? e : e.id) === item.id);
                const canAfford = (char.dinheiro || 0) >= item.preco;
                const isProficient = checkProficiency(item);
                
                return (
                  <motion.div 
                    layout key={item.id}
                    onClick={() => toggleItem(item)}
                    className={`group p-5 rounded-[2.5rem] border-2 transition-all cursor-pointer flex flex-col gap-4 relative overflow-hidden ${
                      isOwned 
                        ? 'bg-amber-950/15 border-amber-500/40 shadow-xl shadow-amber-900/10' 
                        : 'bg-gray-900/60 border-white/5 hover:border-amber-500/20'
                    } ${!isOwned && !canAfford ? 'opacity-40 grayscale pointer-events-none' : ''}`}
                  >
                    {!isProficient && (
                      <div className="absolute top-4 left-4 z-10">
                        <span className="bg-rose-500 text-white text-[8px] font-black px-2 py-0.5 rounded-full shadow-lg flex items-center gap-1">
                          ⚠️ SEM PROFICIÊNCIA
                        </span>
                      </div>
                    )}
                    ...
                    <div className="flex items-start justify-between">
                      <div className="w-12 h-12 rounded-2xl bg-gray-950 border border-white/5 flex items-center justify-center text-2xl group-hover:scale-110 transition-transform">
                        {category === 'arma' ? '⚔️' : category === 'armadura' ? '🛡️' : '📦'}
                      </div>
                      <span className={`text-[10px] font-black px-3 py-1 rounded-full uppercase tabular-nums ${isOwned ? 'bg-amber-500 text-black' : 'bg-gray-950 text-amber-500 border border-amber-500/20'}`}>
                        T$ {(item.preco || 0).toLocaleString('pt-BR')}
                      </span>
                    </div>

                    <div>
                      <h4 className="font-black text-white text-base tracking-tight truncate leading-none mb-1">{item.nome}</h4>
                      <div className="flex flex-wrap gap-2 mt-2">
                        {item.dano && (
                           <span className="text-[9px] font-black bg-rose-500/10 text-rose-400 px-2 py-0.5 rounded-lg border border-rose-500/10">💥 {item.dano}</span>
                        )}
                        {item.def && (
                           <span className="text-[9px] font-black bg-sky-500/10 text-sky-400 px-2 py-0.5 rounded-lg border border-sky-500/10">🛡️ +{item.def}</span>
                        )}
                        <span className="text-[9px] font-black bg-slate-800 text-slate-400 px-2 py-0.5 rounded-lg border border-white/5">⚖️ {item.peso || 1}kg</span>
                      </div>
                    </div>

                    <div className="mt-auto pt-4 border-t border-white/5 flex items-center justify-between">
                       <span className={`text-[9px] font-black uppercase tracking-widest ${isOwned ? 'text-amber-500' : 'text-slate-600'}`}>
                         {isOwned ? 'Adquirido' : 'Disponível'}
                       </span>
           ...
           <div className="sticky top-6 flex flex-col gap-6">
              <div className="flex items-center justify-between px-2">
                 <h3 className="text-xs font-black text-white uppercase tracking-[0.3em] flex items-center gap-2">
                   🎒 Mochila
                   <span className="text-[10px] text-slate-600 font-bold">({(char.equipamento || []).length})</span>
                 </h3>
              </div>
              ...
              <div className="flex flex-col gap-3 max-h-[60vh] overflow-y-auto pr-2 custom-scrollbar">
                {(char.equipamento || []).length === 0 && (
                  <div className="p-8 border-2 border-dashed border-white/5 rounded-[2rem] text-center">
                    <p className="text-[10px] text-slate-600 font-black uppercase tracking-widest leading-relaxed">Sua jornada começa com a bolsa vazia.</p>
                  </div>
                )}
                {(char.equipamento || []).map((e, idx) => {
                  const item = ITENS[typeof e === 'string' ? e : e.id];
                  if (!item) return null;
                  const uid = typeof e === 'string' ? e : e.uid;
                  const isProficient = checkProficiency(item);
                  return (
                    <motion.div 
                      key={uid} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}
                      className="group bg-gray-900 border border-white/10 p-4 rounded-2xl hover:border-amber-500/30 transition-all flex flex-col gap-3 relative overflow-hidden"
                    >
                      <div className="flex items-center justify-between gap-3">
                         <div className="flex flex-col min-w-0">
                            <span className="text-[11px] font-black text-white uppercase truncate">{item.nome}</span>
                            {!isProficient && <span className="text-[7px] font-black text-rose-500 uppercase tracking-tighter">⚠️ Sem Proficiência</span>}
                         </div>
                         <button onClick={() => toggleItem(item)} className="w-8 h-8 rounded-xl bg-gray-950 border border-white/5 flex items-center justify-center text-xs text-rose-500 hover:bg-rose-500 hover:text-white transition-all">✕</button>
                      </div>

                      {(e.mods?.length > 0 || e.material) && (
                        <div className="flex flex-wrap gap-1">
                           {e.mods?.map(m => <span key={m} className="text-[7px] bg-amber-500/10 text-amber-500 border border-amber-500/20 px-1.5 py-0.5 rounded italic">
                           ...
                           {e.material && <span className="text-[7px] bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 px-1.5 py-0.5 rounded italic">{MATERIAIS[e.material]?.nome}</span>}
                        </div>
                      )}

                      <button 
                        onClick={() => setCustomizingItem(e)}
                        className="w-full py-2 bg-gray-950 border border-white/5 hover:border-amber-500/40 text-[9px] font-black text-slate-400 hover:text-amber-500 rounded-xl transition-all uppercase tracking-widest"
                      >
                        🔧 Customizar Item
                      </button>
                    </motion.div>
        ...
        {customizingItem && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="px-4 md:px-8 py-2 border-b border-white/5 flex gap-4 bg-gray-950/40">
                <button 
                  onClick={() => setModalTab('mods')}
                  className={`px-4 py-2 text-[10px] font-black uppercase tracking-widest border-b-2 transition-all ${modalTab === 'mods' ? 'border-amber-500 text-amber-500' : 'border-transparent text-slate-500 hover:text-slate-300'}`}
                >
                  Melhorias
                </button>
                <button 
                  onClick={() => setModalTab('mat')}
                  className={`px-4 py-2 text-[10px] font-black uppercase tracking-widest border-b-2 transition-all ${modalTab === 'mat' ? 'border-indigo-500 text-indigo-500' : 'border-transparent text-slate-500 hover:text-slate-300'}`}
                >
                  Materiais
                </button>
              </div>
                               ...
                               return (
                                 ...
                                 >
                                   <div className="min-w-0 pr-2">
                                     <p className={`text-xs font-bold truncate ${isSelected ? 'text-amber-500' : 'text-white'}`}>
                                       {mod.nome} {isSelected && '✓'}
                                     </p>
                                     <p className="text-[9px] text-slate-500 line-clamp-1">{mod.efeito}</p>
                                   ...
                                   {!isSelected ? (
                                     <span className="text-[10px] font-black text-amber-600 bg-amber-600/10 px-2 py-1 rounded-lg shrink-0">
                                       {isFree ? 'GRÁTIS' : `T$ ${addCost}`}
                                     </span>
                                   ) : (
                                     <span className="text-[10px] font-black text-rose-500 bg-rose-500/10 px-2 py-1 rounded-lg group-hover:block hidden shrink-0">
                                       REMOVER
                                     </span>
                                   )}
                                 </button>
